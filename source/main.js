...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    hiddenCard.style.transform =
      "translateX(30px) translateY(20px) scale(0.95) rotateZ(1deg)";
  }, 100);
  ...
  function onDragEnd(e) {
    ...
    if (Math.abs(currentX) > threshold) {
      ...
      if (!isMuted) {
        const cardShuffleSound = new Audio(
          "assets/soundEffects/cardShuffle.mp3",
        );
  ...
  calendar.onDatePicked = (date) => {
    if (!App.getFileStore().getFile("journals")) {
      App.getFileStore().createDirectory("journals");
    }
  ...
  let file = App.getFileStore().getFile("scratch.txt");
  if (file) {
    textarea.value = file.getContent();
  }
  textarea.addEventListener("input", () => {
    ...
    if (content) {
      let file = App.getFileStore().getFile("scratch.txt");
      if (!file) {
        file = App.getFileStore().createFile("scratch.txt");
      }
      file.setContent(content);
      App.getFileStore().sync();
    }
  });
